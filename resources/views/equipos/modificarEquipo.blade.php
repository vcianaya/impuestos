...
@section('cuerpo')
<div class="titulo">MODIFCAR DE EQUIPO </div>
<form name="frmregusuario" id="frmregusuario" action="{!!url('guardar/equipo')!!}" method="post">
	...
	<legend>DATOS DEL EQUIPO</legend>
		...
		<tr style="height:30px;">
			...
			<td width="480px"><input type="text" name="nom_equ" value="{{$equipo->nom_equ}}" class="txtcampo " placeholder="nombre del equipo">
			<label style="font-size:10px;color:red;">{{ $errors->first('nom_equ') }}</label>
			<input type="hidden" name="x" value="0">
			<input type="hidden" name="id" value="{{$equipo->id_equ}}">

			</td>
		</tr>
		<tr style="height:30px;">
			<td width="90px" class="lblnombre">MARCA :</td>
			<td width="480px"><input type="text" name="mar_equ" value="{{$equipo->mar_equ}}" class="txtcampo " placeholder="Marca">
			<label style="font-size:10px;color:red;">{{ $errors->first('mar_equ') }}</label>
			</td>
		</tr>
		<tr style="height:30px;">
			<td width="90px" class="lblnombre">MODELO :</td>
			<td width="480px"><input type="text" name="mod_equ" value="{{$equipo->mod_equ}}" class="txtcampo " placeholder="Modelo">
			<label style="font-size:10px;color:red;">{{ $errors->first('mod_equ') }}</label>
			</td>
		</tr>
		<tr style="height:30px;">
			<td width="90px" class="lblnombre">SISTEMA OPERATIVO :</td>
			<td width="480px"><input type="text" name="so_equ" value="{{$equipo->so_equ}}" class="txtcampo " placeholder="Sistema Opertivo">
			<label style="font-size:10px;color:red;">{{ $errors->first('so_equ') }}</label>
			</td>
		</tr>
		<tr style="height:30px;">
			<td width="90px" class="lblnombre">IP DEL EQUIPO</td>
			<td width="480px"><input type="text" name="ip_equ" value="{{$equipo->ip_equ}}" class="txtcampo " placeholder="Direccion ip">
		...
		<tr style="height:30px;">
			...
			<td width="480px"><input type="text" name="se_equ" value="{{$equipo->se_equ}}" class="txtcampo " placeholder="">
			<label style="font-size:10px;color:red;">{{ $errors->first('se_equ') }}</label>
			</td>
		</tr>
		<tr style="height:30px;">
			<td width="90px" class="lblnombre">CS_EQU :</td>
			<td width="480px"><input type="text" name="cs_equ"  value="{{$equipo->cs_equ}}" class="txtcampo" placeholder="">
			<label style="font-size:10px;color:red;">{{ $errors->first('cs_equ') }}</label>
			</td>
		</tr>
		<tr style="height:30px;">
			<td width="90px" class="lblnombre">DESCRIPCION:</td>
			<td width="480px"><textarea name="des_equ" value="{{$equipo->des_equ}}"  cols="40" rows="5" placeholder="Descripcion del equipo"></textarea>
				...
				<table>
						<tr style="height: 50px;">
							<td>
                 			...
                 			<a class="botones ico-btnlimpiar" href="{{url('admin/equipo')}}">volver</a>
                 			</td>
						</tr>
					</table>
				</center>
